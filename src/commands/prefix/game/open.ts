...
export default prefix(
  ...
  async (client, user, message, args) => {
    ...
    // Handle soul box case
    const isSoulBox = itemId === client.items.soul_box.id || ["sb", client.items.soul_box.name.toLowerCase()].includes(itemId.toLowerCase());
    if (isSoulBox) {
      if (user.soul_box <= 0) {
        return message.channel.send({
          embeds: [new ErrorInterface(client).setDescription(`Bạn không có đủ ${client.items.soul_box.icon}`)],
        });
      }
    ...
    // Find pack by ID or name
    const item = client.packs.find(
      (pack) =>
        pack.id === itemId || 
        pack.name.toLowerCase() === args.join(" ").toLowerCase() || 
        pack.name.split(" ").map(m => m[0]).join("").toLowerCase() === itemId.toLowerCase()
    );
    ...
    for (let i = 0; i < 3; i++) {
      ...
      // Upsert the card into the user's collection
      ...
      // Notify channel if "SR" rank card is found
