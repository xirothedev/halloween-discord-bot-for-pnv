...
export default prefix(
    ...
    async (client, user, message, args) => {
        ...
        const bxh = `
        <:pnv_bxhev:1293635731503185920> ${bold("BXH")}
        > \`#${(await client.utils.getTopPremiumCandy(user)) + 1}\` ${bold("Top Kẹo Hắc Ám")}
        > \`#${(await client.utils.getWinnerStreak(user)) + 1}\` ${bold("Top Chuỗi Thắng")}
        > \`#${topPower.length + 1}\` ${bold("Top Chiến Lực")}
        `;
