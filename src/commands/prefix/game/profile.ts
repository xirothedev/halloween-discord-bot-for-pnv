...
import { Attachment, AttachmentBuilder, bold } from "discord.js";
...
export default prefix(
    ...
    async (client, user, message, args) => {
        ...
        const powers = await Promise.all(users.map((user) => embed.getTopPower(user)));
        const power = await embed.getTopPower(user);
        ...
        const bxh = `<:pnv_bxhev:1293635731503185920> ${bold("BXH")}\n> \`#${await embed.getTopPremiumCandy()}\` ${bold("Top Kẹo Xanh")}\n> \`#${await embed.getWinnerStreak()}\` ${bold("Top Chuỗi Thắng")}\n> \`#${topPower.length}\` ${bold("Top Lực Chiến")}`;
